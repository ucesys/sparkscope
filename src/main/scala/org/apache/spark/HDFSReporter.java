...
public class HDFSReporter extends ScheduledReporter {
    ...
    public static class Builder {
        ...
        private Builder(MetricRegistry registry) {
            LOGGER.info("Using HDFSReporter");
    ...
    private void report(long timestamp, String name, String header, String line, Object... values) {
        try {
            LOGGER.debug("Writing to {}", name);
            String[] stringArr = name.split("\\.");
            String firstEntry = stringArr[0];
            String appId = "";
            if(!firstEntry.startsWith("app")) {
                LOGGER.warn("log file doesn't start with app!");
                return;
            }
            ...
            {
                LOGGER.warn("Couldn't parse executorId to int!: " + executorId);
                return;
            }
    ...
    private boolean setHadoopConf() {
        ...
        try {
//            fileSystem = Utils.getHadoopFileSystem(directory, configuration);
