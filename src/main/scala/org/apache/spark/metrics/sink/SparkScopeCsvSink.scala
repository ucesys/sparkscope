...
private[spark] class SparkScopeCsvSink(val property: Properties, val registry: MetricRegistry, val mgr: SecurityManager) extends Sink {
  ...
  val reporter: AbstractCsvReporter = CsvReporterBuilder.forRegistry(registry)
    ...
    .build(
      pollDir,
      Option(property.getProperty(CSV_KEY_APP_NAME)),
